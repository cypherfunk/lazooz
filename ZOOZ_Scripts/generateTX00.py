...
def generateTX00(l_json):
    print json.dumps(l_json)
    listOptions = json.loads(str(''.join(l_json)))
    ...
    if available_balance < Decimal(0.00006*3) and not force:
        print json.dumps({ "status": "NOT OK", "error": "Not enough funds" , "fix": "Set \'force\' flag to proceed without balance checks oren1" })
        return -1
    ...
    print "fee_total:",fee_total,"change:",change,"largest_spendable_input[]:" ,largest_spendable_input['amount'] 
    if (Decimal(change) < Decimal(0) or fee_total > largest_spendable_input['amount']) and not force:
        print json.dumps({ "status": "NOT OK", "error": "Not enough funds" , "fix": "Set \'force\' flag to proceed without balance checks oren2" })
        return -1
    ...
    cmd= 'bitcoind sendrawtransaction '+signed_transaction['hex']

    print cmd

    os.system(cmd)
    #result = conn.sendrawtransaction( signed_transaction['hex'])
    #print result
